...
class LocalServiceManager final {
 public:
  ...
  //
  // Returns 0 on failure. Registration can fail if the attribute database has
  // run out of handles or if the hierarchy contains characteristics or
  // descriptors with repeated IDs. Objects under |service| must have unique
  // identifiers to aid in value request handling.
