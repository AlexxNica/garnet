...
TEST(GATT_LocalServiceManagerTest, EmptyService) {
  ...
  EXPECT_EQ(kPrimaryServiceGroupType, iter->group_type());
  ...
  EXPECT_EQ(kSecondaryServiceGroupType, iter->group_type());
  EXPECT_TRUE(common::ContainersEqual(
...
TEST(GATT_LocalServiceManagerTest, RegisterCharacteristic) {
  ...
  constexpr uint8_t kChrcProps = kCharacteristicPropertyRead;
  ...
  EXPECT_EQ(kCharacteristicDeclarationType, attrs[1].type());
...
TEST(GATT_LocalServiceManagerTest, ReadCharacteristicNoReadPermission) {
  ...
  auto service = std::make_unique<Service>(true /* primary */, kTestType16);
  service->AddCharacteristic(std::make_unique<Characteristic>(
      kChrcId, kChrcType16, kCharacteristicPropertyRead, 0, kReadReqs,
      kWriteReqs));
  ...
  // number 3.
  constexpr att::Handle kHandle = 0x0003;
  auto* attr = mgr.database()->FindAttribute(kHandle);
  ASSERT_TRUE(attr);
...
TEST(GATT_LocalServiceManagerTest, ReadCharacteristic) {
  ...
  bool called = false;
  IdType service_id;
  auto read_cb = [&](auto service_id, auto id, auto offset,
                     const auto& responder) {
    called = true;
    EXPECT_EQ(service_id, service_id);
...
TEST(GATT_LocalServiceManagerTest, WriteCharacteristicNoWritePermission) {
  ...
  auto service = std::make_unique<Service>(true /* primary */, kTestType16);
  service->AddCharacteristic(std::make_unique<Characteristic>(
      kChrcId, kChrcType16, kCharacteristicPropertyWrite, 0, kReadReqs,
      kWriteReqs));
...
TEST(GATT_LocalServiceManagerTest, WriteCharacteristic) {
  ...
  bool called = false;
  IdType service_id;
  auto write_cb = [&](auto service_id, auto id, auto offset, const auto& value,
                      const auto& responder) {
    called = true;
    EXPECT_EQ(service_id, service_id);
    EXPECT_EQ(kChrcId, id);
...
TEST(GATT_LocalServiceManagerTest, ReadDescriptorNoReadPermission) {
  ...
  // The first descriptor of the first characteristic of the first service has
  // handle number 4.
  constexpr att::Handle kHandle = 0x0004;
  auto* attr = mgr.database()->FindAttribute(kHandle);
  ASSERT_TRUE(attr);
