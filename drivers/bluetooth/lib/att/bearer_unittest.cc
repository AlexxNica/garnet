...
TEST_F(ATT_BearerTest, RegisterHandlerErrorClosed) {
  ...
  EXPECT_EQ(Bearer::kInvalidHandlerId,
            bearer()->RegisterHandler(kNotification, [](auto) {}));
  EXPECT_EQ(Bearer::kInvalidHandlerId,
            bearer()->RegisterHandler(kWriteCommand, [](auto) {}));
}

TEST_F(ATT_BearerTest, RegisterTransactionHandlerErrorClosed) {
  bearer()->ShutDown();
  EXPECT_FALSE(bearer()->is_open());
  EXPECT_EQ(Bearer::kInvalidHandlerId, bearer()->RegisterTransactionHandler(
                                           kWriteRequest, [](auto, auto) {}));
  EXPECT_EQ(Bearer::kInvalidHandlerId, bearer()->RegisterTransactionHandler(
                                           kIndication, [](auto, auto) {}));
}

TEST_F(ATT_BearerTest, RegisterHandlerErrorInvalidOpCode) {
  EXPECT_EQ(Bearer::kInvalidHandlerId,
            bearer()->RegisterHandler(kWriteRequest, [](auto) {}));
  EXPECT_EQ(Bearer::kInvalidHandlerId,
            bearer()->RegisterHandler(kWriteResponse, [](auto) {}));
  EXPECT_EQ(Bearer::kInvalidHandlerId,
            bearer()->RegisterHandler(kIndication, [](auto) {}));
}

TEST_F(ATT_BearerTest, RegisterTransactionHandlerErrorInvalidOpCode) {
  EXPECT_EQ(Bearer::kInvalidHandlerId, bearer()->RegisterTransactionHandler(
                                           kWriteCommand, [](auto, auto) {}));
  EXPECT_EQ(Bearer::kInvalidHandlerId, bearer()->RegisterTransactionHandler(
                                           kWriteResponse, [](auto, auto) {}));
  EXPECT_EQ(Bearer::kInvalidHandlerId, bearer()->RegisterTransactionHandler(
                                           kNotification, [](auto, auto) {}));
}
...
TEST_F(ATT_BearerTest, RegisterTransactionHandlerErrorAlreadyRegistered) {
  EXPECT_NE(Bearer::kInvalidHandlerId, bearer()->RegisterTransactionHandler(
                                           kIndication, [](auto, auto) {}));
  EXPECT_EQ(Bearer::kInvalidHandlerId, bearer()->RegisterTransactionHandler(
                                           kIndication, [](auto, auto) {}));
}
...
TEST_F(ATT_BearerTest, UnregisterHandler) {
  auto id0 = bearer()->RegisterHandler(kNotification, [](auto) {});
  auto id1 =
      bearer()->RegisterTransactionHandler(kIndication, [](auto, auto) {});
  EXPECT_NE(Bearer::kInvalidHandlerId, id0);
  EXPECT_NE(Bearer::kInvalidHandlerId, id1);

  bearer()->UnregisterHandler(id0);
  bearer()->UnregisterHandler(id1);

  // It should be possible to register new handlers for the same opcodes.
  id0 = bearer()->RegisterHandler(kNotification, [](auto) {});
  id1 = bearer()->RegisterTransactionHandler(kIndication, [](auto, auto) {});
  EXPECT_NE(Bearer::kInvalidHandlerId, id0);
  EXPECT_NE(Bearer::kInvalidHandlerId, id1);
}
...
TEST_F(ATT_BearerTest, RemoteTransactionSeqProtocolError) {
  ...
  bearer()->RegisterTransactionHandler(kTestRequest, handler);
  fake_chan()->Receive(common::CreateStaticByteBuffer(kTestRequest));
...
TEST_F(ATT_BearerTest, RemoteIndicationSeqProtocolError) {
  ...
  bearer()->RegisterTransactionHandler(kIndication, handler);
...
TEST_F(ATT_BearerTest, EndTransactionInvalidPacket) {
  // Empty
  EXPECT_FALSE(
      bearer()->EndTransaction(0, std::make_unique<common::BufferView>()));

  // Exceeds MTU.
  bearer()->set_mtu(1);
  EXPECT_FALSE(bearer()->EndTransaction(0, common::NewBuffer(kTestRequest, 2)));
}

TEST_F(ATT_BearerTest, EndTransactionInvalidId) {
  EXPECT_FALSE(bearer()->EndTransaction(0, common::NewBuffer(kTestResponse)));
}

TEST_F(ATT_BearerTest, EndTransactionWrongOpCode) {
  ...
  EXPECT_FALSE(bearer()->EndTransaction(id, common::NewBuffer(kTestResponse2)));
}

TEST_F(ATT_BearerTest, EndIndicationWrongOpCode) {
  Bearer::TransactionId id;
  bool handler_called = false;
  auto handler = [&id, &handler_called](auto cb_id,
                                        const PacketReader& packet) {
  ...
  EXPECT_FALSE(bearer()->EndTransaction(id, common::NewBuffer(kTestResponse)));
}

TEST_F(ATT_BearerTest, EndTransactionWithResponse) {
  ...
  EXPECT_TRUE(bearer()->EndTransaction(id, common::NewBuffer(kTestResponse)));
...
TEST_F(ATT_BearerTest, IndicationConfirmation) {
  ...
  EXPECT_TRUE(bearer()->EndTransaction(id, common::NewBuffer(kConfirmation)));

  // The transaction is marked as complete.
  EXPECT_FALSE(bearer()->EndTransaction(id, common::NewBuffer(kConfirmation)));
...
TEST_F(ATT_BearerTest, RequestAndIndication) {
  ...
  bearer()->RegisterTransactionHandler(kTestRequest, req_handler);
  bearer()->RegisterTransactionHandler(kIndication, ind_handler);
  ...
  EXPECT_FALSE(
      bearer()->EndTransaction(ind_id, common::NewBuffer(kTestResponse)));
  EXPECT_FALSE(
      bearer()->EndTransaction(req_id, common::NewBuffer(kConfirmation)));

  // It should be possible to end two distinct transactions.
  EXPECT_TRUE(
      bearer()->EndTransaction(req_id, common::NewBuffer(kTestResponse)));
  EXPECT_TRUE(
      bearer()->EndTransaction(ind_id, common::NewBuffer(kConfirmation)));
}

// Test receipt of non-transactional PDUs.
TEST_F(ATT_BearerTest, RemotePDUWithoutResponse) {
  int cmd_count = 0;
  auto cmd_handler = [&cmd_count](const auto& packet) {
  ...
  int not_count = 0;
  auto not_handler = [&not_count](const auto& packet) {
