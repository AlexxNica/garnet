...
Bearer::Bearer(std::unique_ptr<l2cap::Channel> chan,
    ...
    : chan_(std::move(chan)),
      ...
      next_remote_transaction_id_(0u),
      next_handler_id_(1u) {
  FXL_DCHECK(chan_);
...
Bearer::HandlerId Bearer::RegisterHandler(OpCode opcode,
  ...
  MethodType type = GetMethodType(opcode);
  if (type != MethodType::kCommand && type != MethodType::kNotification)
    return kInvalidHandlerId;
...
Bearer::HandlerId Bearer::RegisterTransactionHandler(
    OpCode opcode,
  ...
  MethodType type = GetMethodType(opcode);
  if (type != MethodType::kRequest && type != MethodType::kIndication)
    return kInvalidHandlerId;

  if (transaction_handlers_.find(opcode) != transaction_handlers_.end()) {
    FXL_VLOG(1) << fxl::StringPrintf(
        "att: Can only register one TransactionHandler per opcode (0x%02x)",
        opcode);
    return kInvalidHandlerId;
  }
  ...
  transaction_handlers_[opcode] = handler;
...
void Bearer::UnregisterHandler(HandlerId id) {
  ...
  MethodType type = GetMethodType(opcode);
  if (type == MethodType::kNotification || type == MethodType::kCommand) {
    FXL_CHECK(handlers_.erase(opcode));
  } else if (type == MethodType::kRequest || type == MethodType::kIndication) {
    FXL_CHECK(transaction_handlers_.erase(opcode));
  } else {
    FXL_NOTREACHED() << fxl::StringPrintf(
        "att: unexpected handler opcode: 0x%02x", opcode);
  }
}

bool Bearer::EndTransaction(TransactionId id, common::ByteBufferPtr pdu) {
  FXL_DCHECK(thread_checker_.IsCreationThreadCurrent());
  FXL_DCHECK(pdu);
...
bool Bearer::ReplyWithError(TransactionId id,
  ...
  if (pending_opcode == kIndication) {
    FXL_VLOG(1) << "att: Cannot respond to indication with error!";
    return false;
  }
...
void Bearer::HandleBeginTransaction(RemoteTransaction* currently_pending,
  ...
  auto iter = transaction_handlers_.find(packet.opcode());
  if (iter == transaction_handlers_.end()) {
    FXL_VLOG(1) << fxl::StringPrintf(
