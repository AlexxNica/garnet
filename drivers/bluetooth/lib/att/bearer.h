...
class Bearer final : public fxl::RefCountedThreadSafe<Bearer> {
 public:
  ...
  // Handlers are functions that are invoked when the Bearer receives a PDU that
  // is not tied to a locally initiated transaction (see StartTransaction()).
  // Handlers are invoked on this Bearer's creation thread.
  //
  // The two types of handlers are defined below. The HandlerId is a non-zero
  // integer that can be used to unregister a handler. 0 is an invalid id.
  ...
  using Handler = std::function<void(const PacketReader& packet)>;
  HandlerId RegisterHandler(OpCode opcode, const Handler& handler);

  // TransactionHandler: called when |pdu| is a request or indication. The
  // handler is given a trasaction ID |tid| which can be passed to
  // EndTransaction() to complete a transaction.
  using TransactionHandler =
      std::function<void(TransactionId tid, const PacketReader& packet)>;
  HandlerId RegisterTransactionHandler(OpCode opcode,
                                       const TransactionHandler& handler);

  // Unregisters a handler. |id| cannot be zero.
  void UnregisterHandler(HandlerId id);

  // Ends a currently pending transaction. Returns false if |pdu| is malformed
  // or if |id| and |pdu| do not match a pending transaction.
  bool EndTransaction(TransactionId id, common::ByteBufferPtr pdu);
 ...
 private:
  ...
  std::unordered_map<OpCode, TransactionHandler> transaction_handlers_;
